...
public class ReservationControllerTests {
	...
	// PUT TESTS
	@Test
	public void shouldModifyReservation() throws Exception {
		// Assuming that exists a reservation with resourceId for three days
		String resourceId = UUID.randomUUID().toString();

		Mockito.when(reservationService
				.modifyReservation(Mockito.any(String.class), Mockito.any(Reservation.class)))
		...
		MockHttpServletResponse response = this.mockMvc.perform(put("/reservation/" + resourceId)
				.contentType(MediaType.APPLICATION_JSON)
				.content(reservation.toString()))
				.andReturn()
				.getResponse();
	...
	@Test
	public void shouldNotModifyReservationDoesNotExists() throws Exception {
		String resourceId = UUID.randomUUID().toString();
		Mockito.when(reservationService
				.modifyReservation(Mockito.any(String.class), Mockito.any(Reservation.class)))
				.thenReturn(new Response("Reservation does not exist", null, HttpStatus.NOT_FOUND));
	...
	@Test
	public void shouldNotModifyReservationDifferenceBetweenDateIsLessThanPreviousReservation() throws Exception {
		// Assuming that exists a reservation with resourceId for three days
		String resourceId = UUID.randomUUID().toString();
		Mockito.when(reservationService
				.modifyReservation(Mockito.any(String.class), Mockito.any(Reservation.class)))
	...
	@Test
	public void shouldNotModifyReservationCampReserved() throws Exception {
		String resourceId = UUID.randomUUID().toString();
		Mockito.when(reservationService
				.modifyReservation(Mockito.any(String.class), Mockito.any(Reservation.class)))
				.thenThrow(new OcuppedDateRangeException("The camp is already reserved for that date range."));
